...
const string strMessageMagic = "bluebitcoin Signed Message:\n";
...
static const int64 nTargetTimespan = 1 * 24 * 60 * 60; // bluebitcoin: 1 days
static const int64 nTargetSpacing = 120; // bluebitcoin: 2 minute blocks
                                 ...
                                 const CBlockIndex* pindexBlock, bool fBlock, bool fMiner, bool fStrictPayToScriptHash)
{
    ...
    // fMiner is true when called from the internal bluebitcoin miner
...
bool CheckDiskSpace(uint64 nAdditionalBytes)
{
    ...
    if (nFreeBytesAvailable < nMinDiskSpace + nAdditionalBytes)
    {
        ...
        printf("*** %s\n", strMessage.c_str());
        uiInterface.ThreadSafeMessageBox(strMessage, "bluebitcoin", CClientUIInterface::OK | CClientUIInterface::ICON_EXCLAMATION | CClientUIInterface::MODAL);
        StartShutdown();
        return false;
    }
    return true;
...
bool LoadBlockIndex(bool fAllowNew)
{
    ...
    if (mapBlockIndex.empty())
    {
        ...
        // Genesis block
        const char* pszTimestamp = "Building a coin for Real Estate and Business";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        ...
        block.nVersion = 1;
        block.nTime    = 1393828039; //epochtime
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 1345972;

        if (fTestNet)
        {
            block.nTime    = 1393828039;
            block.nNonce   = 440824;
        }
